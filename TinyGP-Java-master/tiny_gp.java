...
public class tiny_gp {
    ...
    static final int MAX_LEN = 10000, POPSIZE = 25000, DEPTH = 5, GENERATIONS = 25, TSIZE = 2;
    ...
    void stats(double[] fitness, char[][] pop, int gen) {
        ...
        for (i = 0; i < POPSIZE; i++) {
            int current_node_count = traverse(pop[i], 0);
        ...
        favgpop = -totalFitness / POPSIZE; // Calculate average fitness correctly

        System.out.print("Generation=" + gen + " Avg Fitness=" + favgpop + " Best Fitness=" + (-fbestpop) + " Avg Size=" + avg_len + "\nBest Individual: ");
        print_indiv(pop[best], 0); // Print the best individual to the file
        System.out.print("\n");
        System.out.flush();

        saveGenerationData(gen, favgpop, -fbestpop);
    }
    ...
    void saveGenerationData(int gen, double avgFitness, double bestFitness) {
        try {
            generationDataWriter.write(gen + "," + (avgFitness) + "," + (bestFitness) + "\n");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    void evolve() {
        ...
        while (gen < GENERATIONS) {
            if (fbestpop > -3) {
                ...
                // Close the generation data writer
                try {
                ...
                try {
                    ...
                    print_indiv(bestIndividual, 0, bestIndividualWriter);
